...
class ManipulateDoor(Kitchen):
    ...
    def _setup_observables(self):
        ...
        def door_joint_qpos(obs_cache):
            """Door opening state, normalized [0, 1]. 0=closed, 1=fully open."""
            state = self.door_fxtr.get_door_state(env=self)
            return np.array(list(state.values()), dtype=np.float32)

        sensors = [door_joint_qpos]
